...
| **NEW** | [declared-postgres-extensions](PLAN-declared-postgres-extensions.md) | S | **Filed 2026-09-24 (`urb-agents#1446`).** An application needed `pg_trgm`, could not install it, and the request arrived as "run this by hand on production". The extension ships in the image and is simply not installed; the app role is correctly not superuser. 🔴 **And `--init-file` is not the workaround** — it runs psql as the APPLICATION role, so `CREATE EXTENSION` fails there too; anyone trying it first loses an hour. ✅ `configure` already has an admin channel (`_pg_exec` runs as `postgres`), so this is a declaration plus a few lines. ⚠️ **It must be an allowlist**: an extension runs arbitrary C as superuser and the declaration arrives from a registry artifact — the same argument that produced the template allowlist, with more force. |
| **NEW** | [install-reports-disarmed-schedules](PLAN-install-reports-disarmed-schedules.md) | S | **Filed 2026-09-23 (`urb-agents#1439`); the cheap half shipped in 1.6.148.** An install added a seventh cron and left `6 declared / 5 RUNNING / 1 STOPPED` — the new one disarmed — and nothing said so; the 06:00 run did not happen and no surface reported its absence. 🔵 Shipping a schedule stopped is CORRECT and `--start` refusing without `--yes` is correct; neither changes. ⚠️ What is wrong is that **the surprising case and the ordinary one look identical**: on a fresh install everything is stopped and the operator knows, on an upgrade one new thing is stopped among five running and there is no reason to look. Needs the instigator query, which already exists as a playbook and should be reused rather than reimplemented. |
| **NEW** | [api-docs-link-header](PLAN-api-docs-link-header.md) | S | **Filed 2026-09-23 (`urb-agents#1423`); the immediate need is already met at the edge.** An API that answers 406 to `Accept: text/html` and 404 with an error object gives a reader nowhere to go — a `Link: rel="help"` header reaches all of those, because **a header survives where a body is not read**. 🔵 The UIS-native version is worth doing only because the edge version is per-zone and dashboard-managed. ⚠️ **Declared, not derived** — the opposite call from `openapi-server-proxy-uri`, and the rule is: derive when the platform knows, accept a key when only the tenant does. The value must live in the per-app secret or a bare redeploy drops it silently. |
| **NEW** | [secret-version-propagation](PLAN-secret-version-propagation.md) | S–M | **Filed 2026-09-23 from a sweep (`urb-agents#1415`).** Nine pod templates read `urbalurba-secrets` through a `secretKeyRef` and carry nothing that changes when it does, so a re-applied secret reaches no running pod. 🔴 **Two of two examined were defects** — oauth2-proxy found through a rotation that rolled nothing, PostgREST through a published document advertising the wrong hostname — and **neither was found by reading**. Every surface reports success: deploy exits 0, pod healthy, rollout complete, and a process holding a value that is no longer true. ⚠️ Invisible on a fresh install, because the podspec gains the env reference in the same deploy; the installed base is the population at risk. 🔵 A gate already enforces that each is examined and marked; this is the work the markers point at, and `uis secrets apply` owning the roll may replace all nine. |
| **NEW** | [postgres-observability-and-blast-radius](PLAN-postgres-observability-and-blast-radius.md) | S | **Filed 2026-09-21 on Terje's instruction (`urb-agents#1312`), explicitly as backlog — not started, not scheduled.** Two settings for the in-cluster PostgreSQL default: `pg_stat_statements`, because without it a slow query leaves no record of *which query shape* was responsible and it **cannot be added without a restart**; and a finite `temp_file_limit`, because at `-1` one bad analytical join fills the shared data volume and **takes every tenant down instead of failing one query**. 🔵 Neither is urgent — the capacity review that produced them found the instance healthy on every pressure metric. They are filed because both are cheap now and expensive during the incident they prevent. ⚠️ Scope is the CHART only: the external `pg` LXC is ops's, via an ansible drop-in, and UIS cannot set it. |
| **NEW** | The oauth2-proxy OAuth app is owned by a person, not an org | S | **Filed 2026-09-18 from the tester's consolidated report.** The GitHub OAuth app now gating a public host is registered under an individual account. A personal OAuth app **cannot be rotated by anyone else and does not transfer**, so the credential behind a production login gate has a single point of failure that is a person rather than a system. 🔵 Nothing is broken and nothing is urgent — the gate works and the app is correctly configured (`Ov23li` prefix confirms an OAuth App rather than a GitHub App, one callback URL, wildcard matching off). This is the *succession* half: who can rotate the secret after an incident, and what happens when the owner is unavailable. ⚠️ **Re-registering is not free** — a new app means a new client id and secret, one `./uis secrets apply`, and a gate roll, so it is cheapest to do while exactly one host is gated, which is the situation today. The recommendation before the app was created was org-owned; it is worth confirming that was a decision rather than an oversight. |
| **NEW** | `undeploy` leaves the service in autostart, and one stale entry stops the whole boot | S | **Found 2026-09-18 during the oauth2-proxy gate round; the tester inferred it from a printed line, and reading the code made it worse than they claimed.** `cmd_deploy` calls `enable_service` on every successful single-service deploy (`provision-host/uis/manage/uis-cli.sh:504`) and `cmd_undeploy` never calls `disable_service` — the only callers are `cmd_disable` and the toggle in `lib/service-auto-enable.sh:194`. So **undeploy any service and a later bare `./uis deploy` brings it back**, which is already wrong. The sharp edge is what happens when the re-deploy *fails*: `deploy_enabled_services` does `return 1  # Stop on first failure per design` (`lib/service-deployment.sh`), so **one stale entry aborts the boot and every service after it in the list never deploys**. The gate is the concrete case — undeploy it, leave `protected:` empty, and its deploy now refuses by design, so a bare `uis deploy` stops there. ⚠️ **Not fixed in 1.6.125 deliberately**: this is shared-CLI behaviour for every service, it has a symmetry question in it (should `undeploy` disable, or should `deploy` stop auto-enabling?), and it does not belong inside a gate fix. 🔵 Neither half is measured yet — the tester did not run bare `uis deploy` to confirm the abort, and nor have I. Reproduce both before choosing the fix. |
| **NEW** | Traefik v3 — declare it, assert it, and keep the managed install in parity | S–M | **Filed 2026-09-18 from Terje's decision above.** Two pieces, both small, neither done. (1) **Assert the running version** before applying v3 CRDs, and refuse with the reason. Today nothing checks, and `#199`'s failure mode is the worst kind: v3 manifests on a v2 controller 404 every service *while the playbook exits 0*. (2) **Keep the UIS-managed install in parity** — `003-setup-traefik.yml` pins chart 39.0.7 to match what rancher-desktop's k3s ships, and the playbook's note that both sides *"migrate together as one coordinated change"* is a comment rather than a check, so the two can drift silently. ⚠️ **Ranked Tier 1 because a tenant is waiting**: per-path auth policies need `PathRegexp`, which is v3-only, and a service being migrated onto UIS gates restricted pages by a path pattern today. Without the assertion we would be building on an assumption the platform does not verify. 🔵 **Related but not the same as Tier 2 #9 (version-pinning)** — that one is about image tags across services; this is a *minimum platform version* with no enforcement. 🔴 **2026-09-18, and this is now measured rather than argued.** The gate's `PathRegexp` route was tested on a cluster running Traefik **3.7.4** and worked, with no matcher complaint in the log. That is **one host agreeing, not an assertion** — and the same host was on Traefik **2.10.5 a week earlier**, after a Rancher Desktop rebuild silently fell back to bundled k3s v1.25.16. It has flipped **v3 → v2 → v3 in eight days**, without anyone choosing that. On the v2 leg the route would **404 with exit 0 and nothing in the log**. So the failure mode is not hypothetical and the downgrade is not operator error: it arrives with a rebuild. |
