...
week. **See the table below for the recorded instances**, most recently one at 1.6.59. Its words: *"Each was invisible until something failed, and each made the
...
| 1.6.53 | 🔴 `PodNotReady` fired on every **successfully completed** Job pod and could not clear before its 24 h TTL, so on a cluster with a nightly chain the alert group never emptied and re-notified every four hours, permanently | `kube_pod_status_ready{condition="false"} == 1` with no phase exclusion. A Completed pod is 0/1 and ready=false forever — it is not unready, it is finished. The rule's own description said *"too long for a **deployment**"* and the expression enforced nothing of the kind. 🔴 Six false alerts sat beside a **real** one that had been firing four days — a production database with no metrics since a reboot — invisible among alerts that never mean anything (ops, `urb-agents#633`). ✅ **Fix verified end to end** by imac on `#649`: `KubeJobFailed` reached `state=firing` on the failed Job and produced nothing for the successful one; zero `PodNotReady` alerts on four terminal pods where 1.6.46 gave eleven, nine of them on pods that had **succeeded**; and the coverage swap confirmed rather than assumed — the failed pod is excluded from `PodNotReady` **and** its Job is caught by `KubeJobFailed`, which is the only reason the two changes make sense as a pair. ⚠️ Measured on k8s 1.25.16, not the pinned 1.36.3 — the metric is a function of the pinned KSM chart rather than the cluster, but that is reasoning, not measurement |
| 1.6.54 | 🔴 `uis monitors check` printed `ERROR: could not read Uptime Kuma` **and a monitor count**, so a reader took the count as an answer about Kuma. Six dispatched commands — `monitors`, `dagster`, `catalog`, `postgrest`, `browserless`, `neko` — were **absent from `uis help`** | The count is the *intent*, built from the `--from` context and printed before any Kuma read; the error said nothing about what had NOT been compared. ⚠️ It cost a real wrong conclusion reported to Terje: `ops` grepped the help for `monitors`, found nothing, and said Kuma's declarative tooling did not exist (`urb-agents#648`). 🟢 The undocumented-command list came from a **lint**, not from my grep — my ad-hoc version flagged `docs` and `host`, which are documented |
| 1.6.55 | 🔴 `uis deploy prometheus` rendered `chat_id: 0` from an unset variable, destroying a working Alertmanager config — then reported `ok=21 changed=8 failed=0` and **"✓ Prometheus deployed successfully"** while that pod crashlooped. Alerting down eight minutes | Two guards, both self-defeating. The `chat_id: 0` fallback carried a comment saying the alternative *"takes ALL alerting down rather than just delivery"* — Alertmanager treats `0` as absent and refuses the config identically, so **the mitigation caused the outcome it was written to prevent, on every installation without a chat id**. And the readiness wait selected `component=server`, so the one pod whose failure suppresses every other signal was the one never checked (ops, `urb-agents#660`) |
| 1.6.56 | 🔴 The Alertmanager telegram secret was a **required** mount that nothing in the install creates, so a fresh `deploy prometheus` left the pod in `ContainerCreating` **forever** — the quietest failure state there is. 20 hours wedged, `FailedMount x608`, reported by nothing | The same absent secret breaks two things and **1.6.55 fixed one of them**: the config path lost its `telegram_configs`, the volume stayed required. ⚠️ And 1.6.55's guard covered *token set, chat absent* while leaving its **mirror** — chat set, token absent — to hang identically. **One asymmetric case guarded, its twin not** (ops, `urb-agents#693`) |
| 1.6.57 | 🔴 **The 1.6.55 container build failed and nothing said so.** `helm repo add <name>` exits 1 when the name already exists, so a second add broke the image build — 1.6.55 was *released and never published*, `ops` sat two versions behind it, and every fix merged afterwards was stranded on `main` | A non-idempotent command on a shipping path, and **no instrument reports a failed container build at all.** It surfaced only because `uis version` prints *"released but NOT PUBLISHED yet"* — a line added in 1.6.51 for a different reason — and somebody read it. ⚠️ *Fixed* and *in force* were four steps apart on this defect (ops, `urb-agents#693`) |
| 1.6.59 | 🔴 A registry read served from the one-hour cache was **completely silent**, and the symptom was not *"your cache is stale"* but **"the application has not been published yet"** — so the natural next move was to ask the publisher why they were slow | Cost `imac` two rounds and produced one report quoting a template description `dev-templates` had already replaced. ⚠️ And the remediation made it worse: *"run `uis template list`"* reads the **same file**, so it confirms the absence. Fixed by saying what is known — cache age on every read, and a staleness hint at the not-found path where the wrong conclusion is actually drawn — plus `--refresh`, because the old workaround was `docker exec … rm -f /tmp/uis-template-registry*.json` (ops-dev, `urb-agents#716`) |
